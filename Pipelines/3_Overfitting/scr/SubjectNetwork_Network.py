# This file is used to get subject network for individuals based on NSPA.
# Parameters:
...
def getSubjectNet(X_train, y_train, X_test, y_test, edges):
    # formulate testing set delta degree
    out_testing = pn.MakeSubjectNetDatasetEdges(X_train, y_train, X_test, y_test, edges)
    df_out = pd.DataFrame.from_records(out_testing)

    # prepare column names from edges
    col_names = []
    for e in edges:
        col_names.append(str(e[0]) + '+' + str(e[1]))
...
if __name__ == "__main__":
    ...
    df_out = getSubjectNet(data_training_features, data_training_labels, data_trans_features, data_trans_labels, edge_selection)
    df_out.to_csv('FeatrueTrans.csv', sep = ',', index=False, header=True)
